...
proto.create = function() {
  ...
  container.addEventListener( 'mousedown', onContainerPointerStart, { passive: true } );
  container.addEventListener( 'touchstart', onContainerPointerStart, { passive: true } );
  // create canvas
  this.createCanvas();
  // create cursor
...
proto.createCanvas = function() {
  ...
  canvasPointer.on( 'pointerUp', this.canvasPointerUp.bind( this ) );
...
proto.updateColors = function() {
  ...
  for ( i = 0; i < sats; i++ ) {
    var sat = 1 - i/sats;
    // var yOffset = shades * (i-1) + this.satY;
...
proto.open = function() {
  ...
  this.isOpen = true;
  this.updateSizes();
  this.updateCursor(this.color ? (this.colorGrid[ this.color.toUpperCase() ]) : undefined);
  this.renderColors();
  this.setAnchorColor();
...
proto.updateSizes = function() {
  ...
  if(this.canvasOffset.x > 0) this.isOpen = true;
  var cols, rows;
  if ( customLength && !grayCount ) {
    ...
    // rows = Math.max( shades * (sats-1) + this.satY, grayCount );
  }
...
proto.canvasPointerUp = function( event, pointer ) {
  ...
  this.emitEvent( 'select', [ color, swatch.hue, swatch.sat, swatch.lum ] )
}
