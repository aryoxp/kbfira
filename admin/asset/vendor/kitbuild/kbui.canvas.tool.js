class KitBuildCanvasTool {
  constructor(canvas, options) { // console.warn(options)
    this.canvas = canvas
    ...
    this.gridPos = Object.assign({ x: 0, y: 0 }, options ? options.gridPos : null)
  ...
  broadcastEvent(event, data, options) { console.warn(this)
  ...
  hit(position) { // {x: int y: int}
    // console.log(this.toolPos, position)
    if (!this.toolPos || !position) return false
    let hit = (position.x >= this.toolPos.x - this.settings.size/2 
  ...
  actionMulti(event, e, node) {
    console.warn('Tool action for multiple elements is not implemented', e, node)
  }
...
class KitBuildLeftHandleTool extends KitBuildCanvasTool {
  constructor(canvas, options) {
    super(canvas, Object.assign({
      ...
      bdColor: '#EE2222',
      color: "#000",
      iconUnidirectional: '<svg xmlns="http://www.w3.org/2000/svg" width="44" height="44" fill="currentColor" class="bi bi-arrow-right" viewBox="-3 -3 22 22"><path fill-rule="evenodd" d="M1 8a.5.5 0 0 1 .5-.5h11.793l-3.147-3.146a.5.5 0 0 1 .708-.708l4 4a.5.5 0 0 1 0 .708l-4 4a.5.5 0 0 1-.708-.708L13.293 8.5H1.5A.5.5 0 0 1 1 8z"/></svg>',
      iconBidirectional: '<svg xmlns="http://www.w3.org/2000/svg" width="44" height="44" fill="currentColor" class="bi bi-arrow-right" viewBox="-3 -3 22 22"><circle cx="8" cy="8" r="4"/></svg>',
      icon: null
    }, options))
    this.settings.icon = canvas.direction == KitBuildCanvas.BIDIRECTIONAL ? this.settings.iconBidirectional : this.settings.iconUnidirectional
    this.settings.color = canvas.direction == KitBuildCanvas.BIDIRECTIONAL ? this.settings.bdColor : '#000000';
...
class KitBuildDeleteTool extends KitBuildCanvasTool {
  ...
  actionMulti(event, e, nodes) { // console.log(arguments)
    nodes.unselectify()
...
class KitBuildDuplicateTool extends KitBuildCanvasTool {
  ...
  action(event, e, node) { // console.error("ACTION DUPLICATE")
    if (!node) return
    node.unselect()
    ...
    setTimeout(() => {
      ...
      this.canvas.canvasTool.clearIndicatorCanvas()
      this.broadcastEvent(`duplicate-${node.data('type')}`, duplicateNode.json())
      let undoRedo = this.canvas.toolbar.tools.get(KitBuildToolbar.UNDO_REDO) 
      if (undoRedo) undoRedo.post(`duplicate-${node.data('type')}`, {
        undoData: duplicateNode.json(),
        redoData: duplicateNode.json(),
        undo: (canvas, data) => this.canvas.cy.nodes(`#${data.data.id}`).remove(),
        redo: (canvas, data) => {
          this.canvas.cy.add(data)
          this.canvas.applyElementStyle()
        }
  ...
  actionMulti(event, e, nodes) { console.log(nodes.length)
    if (!nodes) return
    ...
    duplicates.forEach(duplicate => {
      ...
      duplicate.data.id = duplicate.data.type == 'concept' 
        ? "c" + this.canvas.getNextConceptId()
        : "l" + this.canvas.getNextLinkId()
    ...
    this.broadcastEvent(`duplicate-nodes`, this.canvas.cy.nodes(ids.join(", ")).jsons())
  }
}
...
class KitBuildSwitchTool extends KitBuildCanvasTool {
  ...
  action(event, e, node) {
    ...
    let undoRedo = this.canvas.canvasTool.getToolbarTool(KitBuildToolbar.UNDO_REDO)
    if (undoRedo) undoRedo
      .post('switch-direction', UndoRedoSwitch.instance({
        undoData: data,
        redoData: data
      }))
    ...
    setTimeout(() =>
      node.unlock().selectify().trigger('select'), 50)
  }
}
...
class KitBuildDisconnectTool extends KitBuildCanvasTool {
  ...
  showOn(what, node) { // console.log(node)
    if (!node) return
    return super.showOn(what, node) && node.connectedEdges('[lock!="locked"]').length
  }

  onMount(e) { // console.log(e)
    this.gridPos.x = -1
    if (e.activeTools.some(t => {
      return t.gridPos.x == -1 && t.gridPos.y == 1 && !(t instanceof KitBuildDisconnectTool)
    })) this.gridPos.x = -2
  }

  action(event, e, node) { // console.warn(event, e, node)
    if (!node) return
...
class KitBuildCreateConceptTool extends KitBuildCanvasTool {
  ...
  action(event, e, node) { // console.log("ACTION")
    ...
    let color = nodeCreateTool ? nodeCreateTool.textColor(backgroundColor) : '#000000';
    ...
    $(floatingLabel).appendTo('body')
      .css('top', $(`#${this.canvas.canvasId}`).offset().top + this.contextRenderedPosition.y - ($(floatingLabel).outerHeight()/2))
      .css('left', $(`#${this.canvas.canvasId}`).offset().left + this.contextRenderedPosition.x - ($(floatingLabel).outerWidth()/2)).on('submit', (e) => {
        e.preventDefault()
        if ($('.kb-node-label').val().trim().length != 0)
          this.createConcept($('.kb-node-label').val(), backgroundColor, this.contextPosition)
        $('.kb-floating-label').remove()
      })
    $('.kb-node-label').focus().select().off('focusout').on('focusout', (e) => {
      e.preventDefault()
      if ($(e.target).val().trim().length != 0)
        this.createConcept($(e.target).val(), backgroundColor, this.contextPosition)
      $('.kb-floating-label').remove()
    })
  }
  ...
  createConcept(label, backgroundColor, position) {
    ...
    if (undoRedo) 
      undoRedo.post(`create-${node.data('type')}`, {
        undoData: `#${node.id()}`,
        redoData: node.json(),
        undo: (canvas, data) => { canvas.cy.nodes(data).remove() },
        redo: (canvas, data) => { canvas.cy.add(data) }
    })
...
class KitBuildCreateLinkTool extends KitBuildCanvasTool {
  ...
  action(event, e, node) { // console.log("ACTION")
    let nodeCreateTool = this.canvas.toolbar.tools.get('node-create')
    let content = ''
    let backgroundColor = nodeCreateTool ? nodeCreateTool.settings.defaultLinkColor : '#dedede'
    ...
    $(floatingLabel).appendTo('body')
      ...
      .css('left', $(`#${this.canvas.canvasId}`).offset().left + this.contextRenderedPosition.x - ($(floatingLabel).outerWidth()/2)).on('submit', (e) => {
        e.preventDefault()
        if ($('.kb-node-label').val().trim().length != 0)
          this.createLink($('.kb-node-label').val(), backgroundColor, this.contextPosition)
        $('.kb-floating-label').remove()
      })
    $('.kb-node-label').focus().select().off('focusout').on('focusout', (e) => {
      e.preventDefault()
      if ($(e.target).val().trim().length != 0)
        this.createLink($(e.target).val(), backgroundColor, this.contextPosition)
      $('.kb-floating-label').remove()
    })
  }
...
class KitBuildLockTool extends KitBuildCanvasTool {
  constructor(canvas, options) {
    super(canvas, Object.assign({
      ...
      colorLock: "#cc0000",
      colorUnlock: "#2e908c",
      iconLock: '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-textarea-t" viewBox="-5 -5 26 26"><path d="M8 1a2 2 0 0 1 2 2v4H6V3a2 2 0 0 1 2-2zm3 6V3a3 3 0 0 0-6 0v4a2 2 0 0 0-2 2v5a2 2 0 0 0 2 2h6a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2z"/></svg>',
      iconUnlock: '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-textarea-t" viewBox="-5 -5 26 26"><path d="M11 1a2 2 0 0 0-2 2v4a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2H3a2 2 0 0 1-2-2V9a2 2 0 0 1 2-2h5V3a3 3 0 0 1 6 0v4a.5.5 0 0 1-1 0V3a2 2 0 0 0-2-2z"/></svg>',
      gridPos: { x: 0, y: -1 }
    }, options))
  ...
  changeState(st = 'lock') {
    this.settings.color = (st == 'lock') ? this.settings.colorLock : this.settings.colorUnlock;
    this.settings.icon = (st == 'lock') ? this.settings.iconLock : this.settings.iconUnlock;
    this.state = st
  }

  onMount(e) { 
    if (e.target.isEdge && e.target.isEdge()) { // console.log(this, e.target.data('lock'))
  ...
  actionMulti(event, e, elements) { // console.log("ACTION", nodes, this.state)
    elements.unselectify()
    let edges = elements.filter('edge').data('lock', "locked")
    this.canvas.canvasTool.clearIndicatorCanvas()
    let ids = []; edges.forEach(e => ids.push(`#${e.id()}`));
    let undoRedo = this.canvas.canvasTool.getToolbarTool(KitBuildToolbar.UNDO_REDO)
    if (undoRedo) undoRedo
      .post(`lock-edges`, {
        ...
        undo: (canvas, data) =>
          canvas.cy.edges(data.edges.join(", ")).data('lock', data.lock).source().trigger('select'),
        redo: (canvas, data) => 
          canvas.cy.edges(data.edges.join(", ")).data('lock', data.lock).source().trigger('select')
      })
  ...
  action(event, e, nodes) { // console.log(event, e, nodes)
    ...
    let undoData = { id: edge.id(), lock: edge.data('lock') == 'locked' ? "unlocked" : "locked" }
    let undoRedo = this.canvas.canvasTool.getToolbarTool(KitBuildToolbar.UNDO_REDO)
    if (undoRedo) undoRedo
      .post(this.state, {
        undoData: undoData,
        redoData: redoData,
        undo: (canvas, data) =>
          canvas.cy.edges(`#${data.id}`).data('lock', data.lock).source().trigger('select'),
        redo: (canvas, data) => 
          canvas.cy.edges(`#${data.id}`).data('lock', data.lock).source().trigger('select')
      })
...
class KitBuildUnlockTool extends KitBuildCanvasTool {
  ...
  actionMulti(event, e, elements) { // console.log("ACTION", nodes)
    elements.unselectify()
...
class KitBuildFocusTool extends KitBuildCanvasTool {
  constructor(canvas, options) {
    super(canvas, Object.assign({
      ...
      colorHide: "#dc3545",
      colorShow: "#198754",
      iconHide: '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-eye-slash" viewBox="-4 -4 24 24"><path d="m10.79 12.912-1.614-1.615a3.5 3.5 0 0 1-4.474-4.474l-2.06-2.06C.938 6.278 0 8 0 8s3 5.5 8 5.5a7.029 7.029 0 0 0 2.79-.588zM5.21 3.088A7.028 7.028 0 0 1 8 2.5c5 0 8 5.5 8 5.5s-.939 1.721-2.641 3.238l-2.062-2.062a3.5 3.5 0 0 0-4.474-4.474L5.21 3.089z"/><path d="M5.525 7.646a2.5 2.5 0 0 0 2.829 2.829l-2.83-2.829zm4.95.708-2.829-2.83a2.5 2.5 0 0 1 2.829 2.829zm3.171 6-12-12 .708-.708 12 12-.708.708z"/></svg>',
      iconShow: '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-eye" viewBox="-4 -4 24 24"><path d="M10.5 8a2.5 2.5 0 1 1-5 0 2.5 2.5 0 0 1 5 0z"/><path d="M0 8s3-5.5 8-5.5S16 8 16 8s-3 5.5-8 5.5S0 8 0 8zm8 3.5a3.5 3.5 0 1 0 0-7 3.5 3.5 0 0 0 0 7z"/></svg>',
      gridPos: { x: -1, y: -1 }
    }, options))
  ...
  changeState(st = 'show') {
    this.settings.icon = st == 'show' ? this.settings.iconHide : this.settings.iconShow
    this.settings.color = st == 'show' ? this.settings.colorHide : this.settings.colorShow
    this.state = st
  }

  action(event, e, nodes) { // console.log(event, e, nodes)
    if (this.state == 'show') {
      this.canvas.cy.elements().not(nodes.neighborhood()).not(nodes).lock().unselectify().panify().addClass('hide')
      this.changeState('hide')
      return
    }
    this.canvas.cy.elements().not(nodes.neighborhood()).not(nodes)
      .unlock().selectify().removeClass('hide').filter('node').unpanify()
    this.changeState('show')
    return
  }
...
class KitBuildCanvasToolCanvas {
  constructor(canvas, options) {
    ...
    this.observer.observe(
      document.querySelector(`#${$(this.canvas.cy.container()).attr("id")} > div`),
      { attributes: true }
    );
  ...
  broadcastEvent(evt, data, options) {
    this.eventListeners.forEach((l, k, map) => { // console.error(l)
  ...
  resizeCanvas() {
    ...
    $(`#${this.settings.indicatorCanvasId}`).css("height", sibling.css("height"));
  ...
  clearCanvas() { // console.error("CLEAR");
  ...
  drawTool(tool, position) {
    ...
    this.preloadIconImagesOfTools([tool]).then(() => this.drawToolAt(tool, position))
    return this
  }
  ...
  drawTools(node) {
    ...
    this.tools.forEach(t => {
      if (t.showOn(node.data('type') == 'link' 
        ? KitBuildCanvasTool.SH_LINK 
        : KitBuildCanvasTool.SH_CONCEPT, node)) {
        t.onMount({
          target: node,
      ...
      // mount it here...
      if (t.showOn(KitBuildCanvasTool.SH_EDGE) && this.canvas.cy.edges(':selected').length) {
    ...
    if (node.data('type') == 'link' && this.settings.connectorEnabled) { // console.log(this.activeTools)
      ...
      node.connectedEdges().forEach(edge => {
        ...
        let tool = edge.data('type') == "left" ? 
        new KitBuildLeftHandleTool(this.canvas, {gridPos: {x: -1, y: 0}, toolPos: position}) :
        new KitBuildRightHandleTool(this.canvas, {gridPos: {x: 1, y: 0}, toolPos: position})
        tool.onMount({
          target: node,
          activeTools: this.activeTools
        })
      ...
      if (node.connectedEdges('[type="left"]').length == 0) {
        // console.log("Draw")
        let lTool = new KitBuildLeftHandleTool(this.canvas, {gridPos: {x: -1, y: 0}})
        lTool.onMount({
          target: node,
          activeTools: this.activeTools
        })
      ...
      if (node.data('limit') === null || 
        (Number.isInteger(node.data('limit')) && 
        node.connectedEdges('[type="right"]').length < node.data('limit'))) {
          let rTool = new KitBuildRightHandleTool(this.canvas, {gridPos: {x: 1, y: 0}})
          rTool.onMount({
            target: node,
            activeTools: this.activeTools
          })
    ...
    this.activeTools.forEach(t => t.preRender({
      target: node,
      activeTools: this.activeTools
    }))
  ...
  drawToolAt(tool, pos) { // console.warn("DRAW TOOL AT", tool, pos)
    ...
    if (tool.settings.type == "handle") {
      ...
      let edge = this.activeNode.connectedEdges(`[type="${tool.settings.which}"][target="${tool.assignedTo}"]`)
      if (edge.length) {
  ...
  onTapStart(e) {
    ...
    this.hoveredConcept    = (this.tapStartOn && this.tapStartOn.type == "concept")
        ? this.tapStartOn 
        : null

    if (!this.tapStartOn && (e.target.data('type') == "left" || e.target.data('type') == "right")) 
    ...
    if (this.tapStartTool || this.tapStartMultiTool) { // console.log(this.tapStartOn)
    ...
    if (this.tapStartTool && this.tapStartTool.settings.type == "handle") {
      ...
      if (this.tapStartOn.length && this.tapStartOn.isNode()) {
        ...
        if (this.candidateEdge) {
          ...
          this.canvas.cy.nodes(`#${this.hoveredConcept.id()}`).lock().unselectify();
        }
      }
    }
  ...
  onTapDrag(e) { // console.warn('tapDrag')
    this.isTapStart = false
    ...
    if (this.dragNode) {
      ...
      if (this.hoveredConcept) { // console.log('hovered')
        // check whether drag position is outside hovered concept
        if (!this.hitBoundingBox(e.position, this.hoveredConcept.boundingBox())) {
  ...
  onTapEnd(e) {
    ...
    if (this.isNodeTapHold) { this.isNodeTapHold = false; return; }
    if (this.isEdgeTapHold) { this.isEdgeTapHold = false; return; }
    ...
    if (!this.tapStartTool && this.tapStartOn.length && e.target.isNode && e.target.isNode())
      this.activeNode = e.target

    // if there is a dragEdge and dragNode of an active node
    // then it is dragging a connection handle
    if (this.dragEdge && this.dragNode && this.activeNode && this.tapStartTool) {
      ...
      if (this.candidateEdge && this.hoveredConcept) {
        ...
        let existingTargetEdge = this.activeNode.connectedEdges(
          `[type="${this.tapStartTool.settings.which}"][target="${this.hoveredConcept.id()}"]`
        );

        // if edge to be created does not exists, create it 
        if (existingTargetEdge.length == 0) {
          let previousEdgeData = edge.length ? Object.assign({}, edge.data(), {id: undefined}) : {}
          ...
          if (edge.length) {
            ...
            if (undoRedo) undoRedo
            .post(`move-connect-${newEdge.data('type')}`, UndoRedoMoveConnect.instance({
              undoData: moveData,
              redoData: moveData
            }))
            edge.remove(); // remove prior edge
            this.broadcastEvent(`move-connect-${newEdge.data('type')}`, moveData)
          } else { // it is just simple connect
            if (undoRedo) undoRedo
            .post(`connect-${newEdge.data('type')}`, UndoRedoConnect.instance({
              undoData: newEdge.json(),
              redoData: newEdge.json()
            }))
            this.broadcastEvent(`connect-${newEdge.data('type')}`, newEdge.json())
          }
        }
      } else { // it was not connected, then just remove it.
        if (edge) {
          if (undoRedo) undoRedo
            .post(`disconnect-${edge.data('type')}`, UndoRedoDisconnect.instance({
              undoData: edge.json(),
              redoData: edge.json()
            }))
    ...
    if (this.tapStartTool) { 
      ...
      } else { this.onToolClicked(this.tapStartTool, e) }
    ...
    if (this.tapStartMultiTool) {
      ...
      let selectedElements = this.canvas.cy.elements(':selected').not('#VIRTUAL')
  ...
  onTapHold(e) { 
    ...
    if (this.tapStartTool === false && 
      this.tapStartMultiTool === false && 
      this.tapStartOn === false) { // tapHold on nothing
        this.activeTools = []
        this.tools.forEach((t) => {
  ...
  onViewport(e) { // console.warn("viewport");
    this.clearCanvas();
    this.clearIndicatorCanvas()
    // this.canvas.cy.nodes('[type="link"]').forEach(l => this.drawLinkRemaining(l))
  ...
  onRender(e) { 
    ...
    if (new Date().getTime() - this.startRender < 400) clearTimeout(this.renderTimeout)
    this.renderTimeout = setTimeout(() => { // console.warn("RENDER DRAW")
      if (!this.isNodeDrag && !this.isTapStart) { // only draw while not dragging or tapstart a node
        this.clearCanvas()
        if (this.canvas.cy.nodes(":selected").length == 1) this.drawTools(this.canvas.cy.nodes(":selected")[0])
        else if (this.canvas.cy.nodes(":selected").length > 1) this.drawSelectedNodesBoundingBox(e)
      }
  ...
  onNodeTapStart(e) { // console.warn("NODE TAP START", e.target)
    if (e.target.hasClass && e.target.hasClass('hide')) return

    // add e.target because it might not have been selected on start drag
    this.canvas.cy.nodes(':selected').add(e.target).not('.hide').forEach(node => {
      node.initialPosition = Object.assign({id: node.id() }, node.position())
  ...
  onNodeTap(e) {
    ...
    if(this.canvas.cy.nodes(":selected").length &&
      this.canvas.cy.nodes(":selected").length == 1) { 
      if(!e.originalEvent) return
      if((this.canvas.cy.nodes(":selected")[0]).id() == e.target.id() &&
  ...
  onNodeDragFreeOn(e) {
    ...
    if (!e.target.selected()) { // dragging unselected node
    ...
    if( selectedNodes.length > 1 ) { // move groups
      let priorMoves = [];
      let laterMoves = [];
      ...
      if (undoRedo) undoRedo.post(`move-nodes`, UndoRedoMoves.instance({
    ...
    e.target.moveToPosition = Object.assign({id: e.target.id() }, e.target.position())
    ...
    let undoRedo = this.getToolbarTool(KitBuildToolbar.UNDO_REDO)
    if (undoRedo)
      undoRedo.post(`move-${e.target.data('type')}`, UndoRedoMove.instance({
        id: e.target.id(),
        undoData: moveData,
        redoData: moveData,
      }))
  ...
  onEdgeTap(e) {
    ...
    setTimeout(() => { // wait for link unselected because of this tap on edge
      if (e.target.selected()) { // if the edge becomes selected
  ...
  onMultiNodeToolClicked(tool, e, elements) {
    ...
    if (typeof tool.actionMulti == "function") tool.actionMulti("click", e, elements);
  }
  ...
  drawDragEdge(dragNode, activeNode) { //console.log(dragNode, activeNode, this.tapStartTool)
    if (!dragNode) return
    ...
    let arrowShape = this.tapStartTool.settings.which == "right" ? 
      "triangle" : "circle"
    if (this.canvas.direction == KitBuildCanvas.BIDIRECTIONAL)
      arrowShape = "circle"
  ...
  getToolPositionOfNode(tool, rect) { // console.log(tool, node)
    /** 
     * rect: {
  ...
  drawCandidateEdgeOnConcept(concept) {
    ...
    this.candidateEdge = this.canvas.cy
      .add({
        ...
        "line-color",
        this.tapStartTool.settings.bdColor.substring(0, 7)
      );
  ...
  drawSelectedNodesBoundingBox(e) { 
    ...
    this.multiNodeTools.forEach(t => {
      if (t.showOnMulti(this.canvas.cy.nodes(':selected'), this.canvas.cy.edges(':selected')))
        this.activeMultiTools.push(t)
    })
    // console.error("DRAW BOUNDING BOX", this.canvas.cy.nodes(':selected').length)
    this.preloadIconImagesOfTools(this.activeMultiTools)
      .then(() => {
  ...
  hitNodes(position, type) { // console.log(type)
    if (!this.workingNodes) this.workingNodes = 
      this.canvas.cy.nodes().not('#VIRTUAL').toArray();
    for (let n of this.workingNodes) {
      ...
      if (this.hitBoundingBox(position, n.boundingBox()) && !n.hasClass('hide')) {
  ...
  hitActiveTools(renderedPosition) { 
    for (let t of this.activeTools) { // console.log(t, renderedPosition, t.hit(renderedPosition))
  ...
  preloadIconImagesOfTools(tools) {
    let promises = []
    for(let t of tools) { // preload all icon images // console.error("PRELOAD", t, t.gridPos.x)
  ...
  drawLinkRemaining(link) { // console.log(link.data())
    if (!link || link.data('type') != "link" || this.canvas.cy.zoom() < 0.65) return
    let remaining = link.data('limit') - link.connectedEdges('[type="right"]').length
...
class UndoRedoMove {
  ...
  undo(canvas, data) { // console.log(data, this)
    canvas.cy.elements(`#${this.id}`).animate({
      position: data.from,
      duration: 100,
      complete: () => 
        canvas.canvasTool.clearCanvas().clearIndicatorCanvas()

    })
  }
  redo(canvas, data) { // console.log(data, this)
    canvas.cy.elements(`#${this.id}`).animate({
      position: data.to,
      duration: 100,
      complete: () => 
