class EncodeDecode {
  ...
  static instance(options) { return new EncodeDecode(options); }
  handleEvent() {
    $('.bt-paste').on('click', e => {
      navigator.clipboard
      .readText()
    ...
    $(".bt-decode").on("click", () => {
      ...
      let index = 0, percent = 0, output = '', empty = 0;
        ...
        result.forEach(r => {
          try {
            ...
            if (typeof decoded === 'object') {
              output += ((index > 1) ? "\n" + JSON.stringify(decoded) : JSON.stringify(decoded));
            } else output += ((index > 1) ? "\n" + decoded : decoded);
          } catch (e) {
            output += ((index > 1) ? "\n" + r : r);
            if (!r || /^\s*$/.test(r)) empty++;
            ...
            $('.progress-value').html(`${index}/${result.length} ${percent}. <small class="text-muted">${empty} empty line(s).</small>`);
    ...
    $('.bt-copy').on('click', e => {
      ...
      navigator.clipboard
        .writeText(cliptext)
        .then(
            success => UI.info('Output content has been copied to clipboard.').show(), 
