class TeachermapApp {
  ...
  initKitCanvas() {
    this.kbui = KitBuildUI.instance(TeachermapApp.kitCanvasId)
    let canvas = this.kbui.canvases.get(TeachermapApp.kitCanvasId)
    canvas.addCanvasTool(KitBuildCanvasTool.FOCUS, { gridPos: { x: 0, y: -1}})
    canvas.addToolbarTool(KitBuildToolbar.CAMERA, { priority: 4 })
    canvas.addToolbarTool(KitBuildToolbar.UTILITY, { priority: 5, trash: false })
    canvas.addToolbarTool(KitBuildToolbar.LAYOUT, { priority: 6 })
    ...
    // Hack for sidebar-panel show/hide
    // To auto-resize the canvas.
    let observer = new MutationObserver((mutations) => $(`#${TeachermapApp.kitCanvasId} > div`).css('width', 0))
    observer.observe(document.querySelector('#admin-sidebar-panel'), {attributes: true})
    // Enable tooltip
    $('[data-bs-toggle="tooltip"]').tooltip({ html: true }) 
    return canvas;
  ...
  handleEvent() {
    ...
    $('#form-search-cmap').on('submit', (e) => { // console.log(e);
      ...
      // console.log(">" + keyword + "<", TeachermapApp.cmapPagination ? TeachermapApp.cmapPagination.keyword : undefined)
      let page = (!TeachermapApp.cmapPagination) ? 1 : TeachermapApp.cmapPagination.page;
      if (TeachermapApp.cmapPagination && keyword != TeachermapApp.cmapPagination.keyword) {
        page = 1;
      }
      if (TeachermapApp.cmapPagination)
        console.log(TeachermapApp.cmapPagination.keyword)
      this.ajax.post(`contentApi/searchConceptMap/${page}/${perpage}`, {
        ...
        if (TeachermapApp.cmapPagination) {
          ...
          TeachermapApp.cmapPagination.page = page;
          TeachermapApp.cmapPagination.update(result.count, perpage);  
        } else {
          TeachermapApp.cmapPagination = 
  ...
  loadTeacherMap(cmid) {
    ...
    Promise.all([KitBuild.openConceptMap(cmid),
      ...
      this.canvas.canvasTool.clearCanvas().clearIndicatorCanvas();
      UI.success('Concept map loaded.').show();
      TeachermapApp.populateKitList(kits);
  ...
  loadKitMap(kid) {
    ...
    KitBuild.openKitMap(kid).then(kitMap => {
      ...
      $('#nav-kit-tab').html(`Kit Map: <span class="text-primary">${kitMap.map.name}</span>`).trigger('click');
